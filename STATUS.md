...
| 05 | Tool Gateway & Connectors | Partial (Milestone M2 added the Tool Registry) | `internal/gateway`, `pkg/mcp`, `pkg/policy`, `pkg/audit`, `pkg/toolregistry`, `cmd/helmdeep-gateway`. Core request handling (MCP listener/client, PDP, hash-chained ledger, credential brokering, taint tracking, aggregate rate limits, obligations) is real. The Tool Registry (§1) enforces its central rule — "no tool executes without being registered; undeclared tools are refused," both in `tools/call` and `tools/list` — with a deliberately narrow schema: risk rating, data classes, required scopes. Not done: JSON Schema input/output validation, the reversibility taxonomy and its runtime-tier/compensation consequences (§1.1), egress destination/payload enforcement (§3, Milestone M3), registry-sourced per-tool limits (§1's `limits` block). See `docs/adr/0008-tool-registry.md`. |
