...
| 02 | System Architecture | Partial | `pkg/types`, `pkg/mcp`, `pkg/policy`, `pkg/audit`; the 4-plane split and Action Record model are real, but only the Data-Plane/Governance slice needed by the Tool Gateway exists. Control Plane §2 has two real components — Tool Registry (`pkg/toolregistry`, Milestone M2) and Agent Registry (`pkg/agentregistry`) — enforcing "undeclared tools/agents are refused"; Model Catalog, Policy Service, Trust Engine, Approval Service, Eval Service, and Tenant/Org Service remain the one `pkg/controlplane` interface. Observability Plane is `pkg/audit` only (no trace collector, metrics/cost engine, or replay service). See `docs/adr/0012-agent-registry.md`. |
...
| 07 | Model Gateway | Partial (Milestone M5) | `pkg/modelgw`. Two real providers (Azure OpenAI, Anthropic), policy-gated via the same `pkg/policy.Decider` and audited to the same `pkg/audit.Store` the Tool Gateway uses — proven directly in `test/e2e/modelgw_test.go`, not just asserted. Explicit version pinning enforced at config time (`"latest"` rejected); fallback between providers is recorded in the ledger, never silent. Not done: Model Catalog, purpose/quality/cost/residency-based routing, the full safety pipeline (PII, prompt-injection detection, structured-output validation, content-safety, leak checks, budget admission), any caching layer, an HTTP/MCP-facing server. See `docs/adr/0011-model-gateway.md`. |
